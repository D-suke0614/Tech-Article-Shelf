...
- [x] `src/pages/index.test.tsx`
  - [x] 初期表示で記事一覧がロードされる（MSWでモック）
  - [x] 記事を検索できる
  - [x] ローディング状態の表示
- [x] TRPCProviderでラップしたテスト用レンダラー作成（`src/test/render-with-providers.tsx`）

### Day 20（1h）：ページ統合テスト（2）✅

- [x] 記事登録フローのテスト
  - [x] フォーム入力 → 送信 → 一覧に追加される
- [x] エラー状態のテスト
  - [x] API失敗時のエラーメッセージ表示
...
- [x] エラーパターンのテスト追加
  - [x] ネットワークエラー
  - [x] タイムアウト
  - [x] 400/401/403/404/500 各ステータスコード
- [x] 振り返り
  - [x] 統合テストで学んだことを整理
...
> **メモ**: Vercelデプロイはユーザー操作が必要なためスキップ
...
- [x] スライド作成（`docs/lt/slides.md` に Markdown 形式で作成）
  - [x] コードは読みやすいフォントサイズで
  - [ ] デモ用のGIF作成（あれば）
...
- [x] 記事執筆（`docs/article/draft.md` に作成）
